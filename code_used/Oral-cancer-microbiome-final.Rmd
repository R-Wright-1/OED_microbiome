---
title: "Oral cancer microbiome 4"
output:
  html_document:
    toc: yes
    toc_float: yes
...
Get table of ASV sequences
```{python}
all_asv = []
tax = pd.read_csv(folder+'processing/taxonomy.csv', index_col=0, header=0)

for record in SeqIO.parse(folder+"qiime2/exports/dna-sequences.fasta", "fasta"):
    all_asv.append([record.id, tax.loc[record.id, 'ASV'], str(record.seq)])

asv_seqs = pd.DataFrame(all_asv, columns=['ASV', 'ASV_name', 'Sequence'])
asv_seqs = asv_seqs.set_index('ASV_name')
asv_seqs = asv_seqs.loc[['ASV'+str(l) for l in range(2644) if l != 0], :]
asv_seqs.to_csv(folder+'processing/asv_sequences.csv')
...
tax_new = pd.read_csv(folder+'processing/taxonomy.csv', index_col=0, header=0)
...
for leaf in leaves[1:]:
  if leaf[0] in genus_dict:
    leaf_name = genus_dict[leaf[0]].replace('g__', '').replace('o__', '')
    if 'Unclassified' not in leaf_name: leaf_name = '  $'+leaf_name+'$'
    else: leaf_name = '  '+leaf_name
    tx = ax_tree.text(leaf[1], leaf[2], leaf_name, va=leaf[4], ha=leaf[5])
...
for t in range(len(sig)):
  ...
  plt.sca(ax)
  gen_name = sig[t].replace('g__', '').replace('c__', '')
  if 'Unclassified' not in gen_name: gen_name = '$'+gen_name+'$'
  ti = plt.title(gen_name, fontweight='bold')
  if t == 0:
    ax.text(-0.4, 1.05, 'A', fontweight='bold', fontsize=20, transform=ax.transAxes)
    ax.text(-0.4, -0.45, 'B', fontweight='bold', fontsize=20, transform=ax.transAxes)
  vals_x, vals_y = [], []
  plotting_x = []
  groups = {}
  for sample in ft.columns:
...
plt.savefig(folder+'figures/Figure3.png', dpi=600, bbox_inches='tight')
# 
#   

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/Figure3.png)
...
for t in range(len(sig)):
  ...
  plt.sca(ax)
  #ti = plt.title(sig[t].replace('s__', '').replace('g__', '').replace('c__', '').replace('_', ' ').capitalize(), fontweight='bold')
  if t == 0:
    ax.text(-0.4, 1.05, 'A', fontweight='bold', fontsize=20, transform=ax.transAxes)
  elif t == 10:
    ax.text(-0.4, -0.45, 'B', fontweight='bold', fontsize=20, transform=ax.transAxes)
  ...
  ti = plt.title(tax[-1]+'\n'+tax[-2].replace('s__', '').replace('g__', '').replace('c__', '').replace('_', ' ').replace('uncultured', 'Uncultured'), fontweight='bold')
  # #plt.yscale('symlog')
...
## PICRUSt import data {.tabset}

Get pathway stratified abundance in square form:
```{python}
pwy_strat = pd.read_csv(folder+'PICRUST/pathways_out/path_abun_contrib.tsv', sep='\t', header=0)
sample_abun = {}
for row in pwy_strat.index:
  func_tax = pwy_strat.loc[row, 'function']+'|'+pwy_strat.loc[row, 'taxon']
  abun = pwy_strat.loc[row, 'taxon_function_abun']
  sample = pwy_strat.loc[row, 'sample']
  if sample in sample_abun:
    sample_abun[sample].append([func_tax, abun])
  else:
    sample_abun[sample] = [[func_tax, abun]]

dfs = []
for sample in sample_abun:
  this_df = pd.DataFrame(sample_abun[sample])
  this_df.columns = ['function', sample]
  this_df = this_df.set_index('function')
  dfs.append(this_df)
  
strat_square = pd.concat(dfs).fillna(value=0)
strat_square = strat_square.groupby(by=strat_square.index, axis=0).sum()
strat_square.to_csv(folder+'processing/pathways_stratified_square.csv')
```

Get EC stratified abundance in square form:
```{python}
ec_strat = pd.read_csv(folder+'PICRUST/EC_metagenome_out/pred_metagenome_contrib.tsv', sep='\t', header=0)
sample_abun = {}
for row in ec_strat.index:
  func_tax = ec_strat.loc[row, 'function']+'|'+ec_strat.loc[row, 'taxon']
  abun = ec_strat.loc[row, 'taxon_function_abun']
  sample = ec_strat.loc[row, 'sample']
  if sample in sample_abun:
    sample_abun[sample].append([func_tax, abun])
  else:
    sample_abun[sample] = [[func_tax, abun]]
...
strat_square = pd.concat(dfs).fillna(value=0)
strat_square = strat_square.groupby(by=strat_square.index, axis=0).sum()
strat_square.to_csv(folder+'processing/ec_stratified_square.csv')
```

### Get feature tables normalised

```{python}
ec = folder+'PICRUST/EC_metagenome_out/pred_metagenome_unstrat.tsv'
pwy = folder+'PICRUST/pathways_out/path_abun_unstrat.tsv'
ec_strat = folder+'processing/ec_stratified_square.csv'
pwy_strat = folder+'processing/pathways_stratified_square.csv'

files = [ec, pwy, ec_strat, pwy_strat]
for f in files:
  if '.tsv' in f: ft = pd.read_csv(f, sep='\t', index_col=0, header=0)
  else: ft = pd.read_csv(f, index_col=0, header=0)
  ft_relabun = ft.copy(deep=True)
  ft_relabun = ft_relabun.divide(ft_relabun.sum(axis=0), axis=1).multiply(100)
  ft_relabun.to_csv(folder+'processing/'+f.split('/')[-1].replace('.', '_relabun.'))
  ft_rclr = ft.copy(deep=True)
  X = ft_rclr.iloc[0:].values
  this_ft_rclr = rclr(X)
  ft_rclr = pd.DataFrame(this_ft_rclr, columns=ft_rclr.columns, index=ft_rclr.index.values).fillna(value=0)
  ft_rclr.to_csv(folder+'processing/'+f.split('/')[-1].replace('.', '_rclr.'))


```

### Rarefy feature tables

```{python}
ft = pd.read_csv(folder+'PICRUST/EC_metagenome_out/pred_metagenome_unstrat.tsv', sep='\t', index_col=0, header=0)
```
...
Sort column names after R:
```{python}
ft_rare = pd.read_csv(folder+'processing/pred_metagenome_unstrat_rare_R.csv', index_col=0, header=0)
col_names = {}
for col in ft_rare.columns:
  col_names[col] = col.replace('X', '')
ft_rare = ft_rare.rename(columns=col_names)
ft_rare.to_csv(folder+'processing/pred_metagenome_unstrat_rare.csv')


ft =  pd.read_csv(folder+'PICRUST/pathways_out/path_abun_unstrat.tsv', sep='\t', index_col=0, header=0)
```
...
Sort column names after R:
```{python}
ft_rare = pd.read_csv(folder+'processing/path_abun_unstrat_rare_R.csv', index_col=0, header=0)
col_names = {}
for col in ft_rare.columns:
  col_names[col] = col.replace('X', '')
ft_rare = ft_rare.rename(columns=col_names)
ft_rare.to_csv(folder+'processing/path_abun_unstrat_rare.csv')

ft = pd.read_csv(folder+'processing/ec_stratified_square.csv', index_col=0, header=0)
```
...
physeq_rare <- rarefy_even_depth(physeq, sample.size = min(sample_sums(physeq)), replace = TRUE, trimOTUs = TRUE, verbose = TRUE)
table_rare = data.frame(otu_table(physeq_rare))
write.csv(table_rare, paste(py$folder, 'processing/ec_stratified_square_rare_R.csv', sep=''))
```

Sort column names after R:
```{python}
ft_rare = pd.read_csv(folder+'processing/ec_stratified_square_rare_R.csv', index_col=0, header=0)
col_names = {}
for col in ft_rare.columns:
  col_names[col] = col.replace('X', '')
ft_rare = ft_rare.rename(columns=col_names)
ft_rare.to_csv(folder+'processing/ec_stratified_square_rare.csv')
```

Export to same format as PICRUSt2 started at:
```{python}
ft_rare = pd.read_csv(folder+'processing/ec_stratified_square_rare.csv', index_col=0, header=0)
#sample	function	taxon function_contrib
new_rows = []
for col in ft_rare.columns:
  for row in ft_rare.index:
    tax_func = row.split('|')
    tax, func = tax_func[0], tax_func[1]
    new_rows.append([col, func, tax, ft_rare.loc[row, col]])

strat_rare = pd.DataFrame(new_rows, columns=['sample', 'function', 'taxon', 'rarefied_function_contrib']).set_index('sample')
strat_rare.to_csv(folder+'processing/ec_stratified_rarefied.csv')
```
...
ec_relabun_fn, pwy_relabun_fn = folder+'processing/pred_metagenome_unstrat_relabun.tsv', folder+'processing/path_abun_unstrat_relabun.tsv'

diversity = ['chao1', 'shannon', 'simpson', 'simpson_e']
dfs = [ec_relabun_fn, pwy_relabun_fn]
name_level = ['EC', 'Pathway']
...
for a in range(len(dfs)):
  ...
  all_alpha = pd.concat(all_alpha).fillna(value=0)
  all_alpha = all_alpha.groupby(by=all_alpha.index, axis=0).sum()
  all_alpha.to_csv(folder+'diversity/alpha_diversity_relabun_'+name_level[a]+'.csv')
...
```{python}
ec_relabun_fn, pwy_relabun_fn = folder+'processing/pred_metagenome_unstrat_relabun.tsv', folder+'processing/path_abun_unstrat_relabun.tsv'
ec_strat_relabun_fn, pwy_strat_relabun_fn = folder+'processing/ec_stratified_square_relabun.csv', folder+'processing/pathways_stratified_square_relabun.csv'

ec_level = [ec_strat_relabun_fn]
pwy_level = [pwy_strat_relabun_fn]

diversity = ['braycurtis']
names = ['braycurtis_relabun']
group_names = ['EC_strat', 'Pathway_strat']
groups = [ec_level, pwy_level]
matrix_names = []
similarity_matrix = []
...
for g in range(len(group_names)):
  this_df, this_df_p = [], []
  for d in range(len(names)):
    df = pd.read_csv(folder+'diversity/'+names[d]+group_names[g]+'_braycurtis_relabun.csv', index_col=0, header=0)
    df_r2 = pd.DataFrame(df.loc[:, 'R2']).rename(columns={'R2':plot_name[d]}).transpose()
    df_p = pd.DataFrame(df.loc[:, 'Pr(>F)']).rename(columns={'Pr(>F)':plot_name[d]}).transpose()
    this_df.append(df_r2)
    this_df_p.append(df_p)
...
plt.savefig(folder+'figures/Adonis_PICRUST_R2_results.png', dpi=600, bbox_inches='tight')
...
Ellipses around progression EC numbers:
```{python, eval=FALSE}
plt.figure(figsize=(10,10))
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
...
distances = pd.read_csv(folder+'diversity/beta_diversity_EC_braycurtis_relabun.csv', index_col=0, header=0)
distances.index = distances.index.map(str)
distances.columns = distances.columns.map(str)
md.index = md.index.map(str)
...
plt.tight_layout()
plt.savefig(folder+'figures/PCoA_EC.png', dpi=600, bbox_inches='tight')

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/PCoA_EC.png)

Ellipses around progression EC numbers stratified:
```{python, eval=FALSE}
plt.figure(figsize=(10,10))
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
...
distances = pd.read_csv(folder+'diversity/beta_diversity_EC_strat_braycurtis_relabun.csv', index_col=0, header=0)
distances.index = distances.index.map(str)
distances.columns = distances.columns.map(str)
md.index = md.index.map(str)
...
plt.tight_layout()
plt.savefig(folder+'figures/PCoA_EC_strat.png', dpi=600, bbox_inches='tight')

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/PCoA_EC_strat.png)

Ellipses around progression pathways:
```{python, eval=FALSE}
plt.figure(figsize=(10,10))
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
...
distances = pd.read_csv(folder+'diversity/beta_diversity_Pathway_braycurtis_relabun.csv', index_col=0, header=0)
distances.index = distances.index.map(str)
distances.columns = distances.columns.map(str)
md.index = md.index.map(str)
...
plt.tight_layout()
plt.savefig(folder+'figures/PCoA_Pathway.png', dpi=600, bbox_inches='tight')

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/PCoA_Pathway.png)
...
distances = pd.read_csv(folder+'diversity/beta_diversity_Pathway_strat_braycurtis_relabun.csv', index_col=0, header=0)
distances.index = distances.index.map(str)
distances.columns = distances.columns.map(str)
md.index = md.index.map(str)
...
plt.tight_layout()
plt.savefig(folder+'figures/PCoA_Pathway_strat.png', dpi=600, bbox_inches='tight')

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/PCoA_Pathway_strat.png)
...
plt.tight_layout()
plt.savefig(folder+'figures/PCoA_EC_distance_groups.png', dpi=600, bbox_inches='tight')
...
```{python, eval=FALSE}
ft = pd.read_csv(folder+'PICRUST/EC_metagenome_out/pred_metagenome_unstrat.tsv', index_col=0, header=0, sep='\t')
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
ft_rare = pd.read_csv(folder+'processing/pred_metagenome_unstrat_rare.csv', index_col=0, header=0)
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_EC_grouped.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_EC.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_EC_grouped.csv", sep=''))
```
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_EC_progressors_specific.csv", sep=''))
```
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_EC_progressors_grouped.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_EC_progressors_grouped.csv", sep=''))
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_EC_progressors_specific_full_model", sep=''), transform = "AST", fixed_effects = c("Total_Number_of_Months_Followed_or_to_Progression"), standardize = FALSE, plot_heatmap = F, plot_scatter = F)
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_EC_progressors_grouped_full_model", sep=''), transform = "AST", fixed_effects = c("Time_to_progression_grouped"), standardize = FALSE, plot_heatmap = F, plot_scatter = F)
```

#### Pathways

```{python, eval=FALSE}
ft = pd.read_csv(folder+'PICRUST/pathways_out/path_abun_unstrat.tsv', index_col=0, header=0, sep='\t')
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
ft_rare = pd.read_csv(folder+'processing/path_abun_unstrat_rare.csv', index_col=0, header=0)
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_pathways.csv", sep=''))
```
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_pathways_grouped.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_pathways.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_pathways_grouped.csv", sep=''))
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_pathways_full_model", sep=''), transform = "AST", fixed_effects = c("Progression"), random_effects=c("Group"), standardize = FALSE, plot_heatmap = F, plot_scatter = F, reference="Progression,NP")
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_pathways_grouped_full_model", sep=''), transform = "AST", fixed_effects = c("Time_to_progression_grouped"), random_effects=c("Group"), standardize = FALSE, plot_heatmap = F, plot_scatter = F, reference="Time_to_progression_grouped,NP")
```

#### Pathways progressors only
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_pathways_progressors_specific.csv", sep=''))
```
...
write.csv(ancom_out_all, paste(py$folder, "differential_abundance/ancom_pathways_progressors_grouped.csv", sep=''))
```
...
results_all <- aldex(reads=ft, conditions = md[,2], mc.samples = 128, test="kw", effect=TRUE, include.sample.summary = FALSE, verbose=T, denom="all")
write.csv(results_all, paste(py$folder, "differential_abundance/aldex_pathways_progressors_grouped.csv", sep=''))
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_pathways_progressors_specific_full_model", sep=''), transform = "AST", fixed_effects = c("Total_Number_of_Months_Followed_or_to_Progression"), standardize = FALSE, plot_heatmap = F, plot_scatter = F)
```
...
feat_table = data.frame(t(ft), check.rows = F, check.names = F, stringsAsFactors = F)
results_all <- Maaslin2(feat_table, md, paste(py$folder, "differential_abundance/maaslin_pathways_progressors_grouped_full_model", sep=''), transform = "AST", fixed_effects = c("Time_to_progression_grouped"), standardize = FALSE, plot_heatmap = F, plot_scatter = F)
```
...
diversity = ['chao1', 'shannon', 'simpson', 'simpson_e']
div_names = ['Chao1 richness', 'Shannon diversity', "Simpson's diversity", "Simpson's evenness"]

md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
md.index = md.index.map(str)
div_EC = pd.read_csv(folder+'diversity/alpha_diversity_relabun_EC.csv', index_col=0, header=0)
div_EC.index = div_EC.index.map(str)
div_path = pd.read_csv(folder+'diversity/alpha_diversity_relabun_Pathway.csv', index_col=0, header=0)
div_path.index = div_path.index.map(str)


with open(folder+'processing/matches.dict', 'rb') as f:
    matches_dict = pickle.load(f)

letters = [['A: EC Alpha diversity', 'B: EC Beta diversity'], ['C: Pathway Alpha diversity', 'D: Pathway Beta diversity']]

ax_beta_EC = plt.subplot2grid((6, 5), (1,0), colspan=3, rowspan=2)
ax_r2_EC = plt.subplot2grid((6, 30), (1,26), colspan=4, rowspan=2)
ax_beta_path = plt.subplot2grid((6, 5), (4,0), colspan=3, rowspan=2)
ax_r2_path = plt.subplot2grid((6, 30), (4,26), colspan=4, rowspan=2)

for z in range(2):
  to_plot = diversity
  plot_names = div_names
  for a in range(len(to_plot)):
    if z == 0: ax = plt.subplot2grid((6, 4), (0,a))
    else: ax = plt.subplot2grid((6, 4), (3,a))
    if a == 0:
      ax.text(-0.4, 1.05, letters[z][0], fontweight='bold', fontsize=16, transform=ax.transAxes)
      ax.text(-0.4, -0.35, letters[z][1], fontweight='bold', fontsize=16, transform=ax.transAxes)
    prog, nonp = [], []
    if z == 0: div = div_EC
    else: div = div_path
    for sample in div.index:
...
for pic in ['EC', 'Pathway']:
  if pic == 'EC': ax_beta, ax_r2 = ax_beta_EC, ax_r2_EC
  else: ax_beta, ax_r2 = ax_beta_path, ax_r2_path
  matrix = pd.read_csv(folder+'diversity/beta_diversity_'+pic+'_braycurtis_relabun.csv', index_col=0, header=0)
  matrix.index = matrix.index.map(str)
  pcoa_results = ordination.pcoa(matrix)
  PC1, PC2 = pcoa_results.samples.loc[:, 'PC1'].values, pcoa_results.samples.loc[:, 'PC2'].values
  prop_explain1, prop_explain2 = pcoa_results.proportion_explained[0], pcoa_results.proportion_explained[1]
  ...
  for name in values_x:
    confidence_ellipse(np.asarray(values_x[name]), np.asarray(values_y[name]), ax=ax_beta, edgecolor=colors[name])
  ax_beta.set_xlabel('PCoA1 ('+format(prop_explain1*100, '.2f')+'%)', fontweight='bold'), ax_beta.set_ylabel('PCoA2 ('+format(prop_explain2*100, '.2f')+'%)', fontweight='bold')
  ax_beta.set_title('Bray-Curtis distance', fontweight='bold')
  handles = [Line2D([0], [0], marker='s', color='w', label=name.capitalize().replace('ssion', 'ssor').replace('No ', 'Non-'), markerfacecolor=colors[name], markersize=12) for name in colors]
  ax_beta.legend(handles=handles, loc='upper right')
  files = ['permanova_'+pic+'_braycurtis_relabun.csv', 'permanova_grouped_'+pic+'_braycurtis_relabun.csv', 'permanova_progressors_'+pic+'_braycurtis_relabun.csv']
  names = ['Progression', 'Progression (grouped)', 'Progressors only']
  rename = {'Progression':'Progression', 'Time_to_progression_grouped':'Progression (grouped)', 'Total_Number_of_Months_Followed_or_to_Progression':'Months to progression', 'patient_age':'Age', 'sex':'Sex', 'ethnicity_details':'Ethnicity', 'TotalWeeklyAlcoholUnits':'Alcohol intake', 'smoking_status':'Smoking status', 'diagnosis':'Grade of dysplasia', 'anatomical_site':'Lesion site'}
  main_rows = [rename[r] for r in rename]
  df_R2, df_p = [], []
...
plt.subplots_adjust(wspace=0.5, hspace=0.4)
plt.savefig(folder+'figures/FigureS3.png', dpi=600, bbox_inches='tight')
...
x = {'no progression':1, 'progression':0}

sig = [s for s in sig_hits]
plt.figure(figsize=(20,15))
locs = [[1,4,1,4,1,4,1,4,1,4], [0,0,1,1,2,2,3,3,4,4]]
for t in range(len(sig)):
  ax = plt.subplot2grid((5,6),(locs[1][t], locs[0][t]), colspan=2)
  plt.sca(ax)
  vals_x, vals_y = [], []
  for sample in ft.columns:
    group = md.loc[sample, 'Time_to_progression_grouped']
  ...
  xl = plt.xlabel('Years to progression')
  #plt.yscale('symlog')
  ax = plt.subplot2grid((5,6),(locs[1][t], locs[0][t]-1))
  plt.sca(ax)
  vals_x, vals_y = [], []
  groups = {}
  for sample in ft.columns:
    group = md.loc[sample, 'Progression']
    years = md.loc[sample, 'Total_Number_of_Months_Followed_or_to_Progression']/12
    if group == 'no progression': continue
    val = ft.loc[sig[t], sample]
    val_con = []
    x_P, x_NP = np.random.normal(x[group], 0.1, 1), np.random.normal( x[group]+1, 0.1, 1)
    for m in matches_dict[str(sample)]:
      ...
      line = plt.plot([x_P, x_NP], [val, vcon], 'k-', alpha=0.05)
      scat = plt.scatter(x_NP, vcon, color=colors['no progression'], alpha=0.5)
    scat = plt.scatter(x_P, val, color=colors[group], alpha=0.5)
    if group in groups: groups[group] = [groups[group][0]+[val], groups[group][1]+val_con]
    else: groups[group] = [[val], val_con]
  for group in groups:
    box = ax.boxplot(groups[group], positions=[x[group], x[group]+1], widths=0.8, showfliers=False)
    for item in ['boxes', 'whiskers', 'fliers', 'medians', 'caps']: plt.setp(box[item], color='k')
  #ti = plt.title(sig[t]+': '+info.loc[sig[t], 'description'], fontweight='bold')
  xl = plt.xticks([0,1],['P', 'NP'])
  if locs[0][t] == 1: yl = plt.ylabel('Relative abundance (%)')


plt.tight_layout()
plt.savefig(folder+'figures/FigureS4.png', dpi=600, bbox_inches='tight')

```

![](/Users/robynwright/Dropbox/Langille_Lab_postdoc/OED_project/analysis/figures/FigureS4.png)

## Investigating PICRUST functional diversity
...
Calculate alpha diversity/distance between samples for top 10 EC numbers at ASV, genus and family levels (just re-running it at different levels):
```{python}
ec_strat = pd.read_csv(folder+'processing/ec_stratified_square_rare.csv', index_col=0, header=0)
...
alpha_metrics = ['chao1', 'shannon', 'simpson', 'simpson_e', 'faith_pd']
beta_metrics = ['weighted_unifrac']
level_name = 'genus'
level_tree = folder+'processing/'+level_name+'_tree.nwk'
tree = Tree(level_tree, format=1)
level = 5
...
for node in tree.traverse("postorder"):
    if node.name in tax_dict:
      level_to_asv[tax_dict[node.name].split('; ')[level]] = node.name
      
tree = read(level_tree, format="newick", into=TreeNode)
  
for hit in sig_hits:
  #if hit != 'EC:1.3.1.31': continue
  keeping = []
  rename = {}
  for row in ec_strat.index:
    if hit in row:
      keeping.append(row)
      rename[row] = row.split('|')[1]
  ec_strat_red = ec_strat.copy(deep=True).loc[keeping, :].rename(index=rename)
  rename_level = {}
  for row in ec_strat_red.index:
    rename_level[row] = tax_dict[row].split('; ')[level]
  ec_strat_red = ec_strat_red.rename(index=rename_level)
  ec_strat_red = ec_strat_red.groupby(by=ec_strat_red.index, axis=0).sum()
  alpha_div_ec = []
  for b in range(len(alpha_metrics)):
    if alpha_metrics[b] == 'faith_pd':
      this_df = ec_strat_red.copy(deep=True)
      this_df = this_df.rename(index=level_to_asv)
      this_df.index = this_df.index.map(str)
      this_df = this_df.groupby(by=this_df.index, axis=0).sum()
      X = this_df.transpose().iloc[0:].values
      this_alpha = alpha_diversity("faith_pd", X, this_df.columns, tree=tree, otu_ids=this_df.index.values)
    else:
      X = ec_strat_red.transpose().iloc[0:].values
      this_alpha = alpha_diversity(alpha_metrics[b], X, ec_strat_red.columns)
    this_alpha = pd.DataFrame(this_alpha).rename(columns={0:alpha_metrics[b]})
    alpha_div_ec.append(this_alpha)
  alpha_div_ec = pd.concat(alpha_div_ec).fillna(value=0)
  alpha_div_ec = alpha_div_ec.groupby(by=alpha_div_ec.index, axis=0).sum()
  alpha_div_ec.to_csv(folder+'diversity/functional_diversity/alpha_diversity_'+hit.replace(':', '.')+'_'+level_name+'.csv')
  for d in range(len(beta_metrics)):
    if 'weighted_unifrac' in beta_metrics[d]:
      this_df = ec_strat_red.copy(deep=True)
      this_df = this_df.rename(index=level_to_asv)
      this_df.index = this_df.index.map(str)
      this_df = this_df.groupby(by=this_df.index, axis=0).sum()
      X = this_df.transpose().iloc[0:].values
      similarities = beta_diversity(beta_metrics[d], X, this_df.columns, tree=tree, otu_ids=this_df.index.values)
      similarities = similarities.to_data_frame()
    else:
      X = ec_strat_red.transpose().iloc[0:].values
      similarities = np.nan_to_num(distance.cdist(X, X, beta_metrics[d]))
    similarities = pd.DataFrame(similarities, columns=ec_strat_red.columns, index=ec_strat_red.columns)
    similarities.to_csv(folder+'diversity/functional_diversity/beta_diversity_'+hit.replace(':', '.')+'_'+level_name+'.csv')
  
do_something = True

```

Looking at genus level, because this is what we did for taxonomy mainly:
```{python}
alpha_metrics = ['chao1', 'shannon', 'simpson', 'simpson_e', 'faith_pd']
plt.figure(figsize=(30,20))

sig_hits_list = [hit for hit in sig_hits]
x = {'no progression':1, 'progression':0}
md = pd.read_csv(folder+'OM_metadata_groups.csv', index_col=0, header=0)
md.index = md.index.map(str)
info = pd.read_csv(folder+'PICRUST/EC_metagenome_out/pred_metagenome_unstrat_descrip.tsv', sep='\t', index_col=0, header=0)
...
for h in range(len(sig_hits_list)):
  alpha_df = pd.read_csv(folder+'diversity/functional_diversity/alpha_diversity_'+sig_hits_list[h].replace(':', '.')+'_genus.csv', index_col=0, header=0)
  alpha_df.index = alpha_df.index.map(str)
  for a in range(len(alpha_metrics)):
    ax = plt.subplot2grid((5,10),(a, h))
    if a == 0: ti = plt.title(sig_hits_list[h]+'\n'+info.loc[sig_hits_list[h], 'description'].replace(' ', '\n'), fontweight='bold')
    if h == 0: yl = plt.ylabel(alpha_metrics[a], fontweight='bold')
    vals_x, vals_y = [], []
    groups = {}
    for sample in alpha_df.index:
      group = md.loc[sample, 'Progression']
      years = md.loc[sample, 'Total_Number_of_Months_Followed_or_to_Progression']/12
      if group == 'no progression': continue
      val =  alpha_df.loc[sample, alpha_metrics[a]]
      val_con = []
      x_P, x_NP = np.random.normal(x[group], 0.1, 1), np.random.normal( x[group]+1, 0.1, 1)
      for m in matches_dict[str(sample)]:
        vcon = alpha_df.loc[m, alpha_metrics[a]]
        val_con.append(vcon)
        line = plt.plot([x_P, x_NP], [val, vcon], 'k-', alpha=0.05)
        scat = plt.scatter(x_NP, vcon, color=colors['no progression'], alpha=0.5)
      scat = plt.scatter(x_P, val, color=colors[group], alpha=0.5)
      if group in groups: 
        groups[group] = groups[group]+[val]
        groups['no progression'] = groups['no progression']+val_con
      else: 
        groups[group] = [val]
        groups['no progression'] = val_con
    for group in groups:
      box = ax.boxplot(groups[group], positions=[x[group]], widths=0.8, showfliers=False)
      for item in ['boxes', 'whiskers', 'fliers', 'medians', 'caps']: plt.setp(box[item], color='k')
    xl = plt.xticks([0,1],['P', 'NP'])
    high, low = float(ax.get_ylim()[1]), float(ax.get_ylim()[0])
    span = high-low
    diff = span*0.01
    high = high+diff*8
    stat, p = mannwhitneyu(groups['progression'], groups['no progression'])
    li = ax.plot([0, 0, 1, 1], [high-diff, high, high, high-diff], color='k')
    string = '\nU='+str(round(stat, 2))+', $p$='+str(round(p, 3))
    tx = ax.text(0.5, high+(diff*0.5), string, va='bottom', ha='center')
    #if z == 0 and a < 3: high = high+span*0.1
    yl = ax.set_ylim([low,high+span*0.1])

print(groups)

plt.savefig(folder+'figures/functional_diversity/sig_ec_genus_prog_nonprog.png', dpi=600, bbox_inches='tight')
```
...
for h in range(len(sig_hits_list)):
  ...
  for a in range(len(alpha_metrics)):
    ...
    for group in groups:
      ...
      high, low = float(ax.get_ylim()[1]), float(ax.get_ylim()[0])
      span = high-low
      high = max([max(groups[group][0]), max(groups[group][1])])
      diff = span*0.01
      high = high+diff*8
      stat, p = mannwhitneyu(groups[group][0], groups[group][1])
      if p <= 0.05:
        li = ax.plot([x[group], x[group], x[group]+1, x[group]+1], [high-diff, high, high, high-diff], color='k')
        string = '\nU='+str(round(stat, 2))+', $p$='+str(round(p, 3))
        tx = ax.text(x[group]+0.5, high+(diff*0.5), '*', va='bottom', ha='center')
        #if z == 0 and a < 3: high = high+span*0.1
        #yl = ax.set_ylim([low,high+span*0.1])
    xl = plt.xticks([x[y]+0.5 for y in x], [y for y in x])

plt.savefig(folder+'figures/functional_diversity/sig_ec_genus_grouped_time.png', dpi=600, bbox_inches='tight')
```
...
sig = [s for s in sig_hits]
plt.figure(figsize=(20,30))
locs = [[1,4,1,4,1,4,1,4,1,4], [0,0,2,2,4,4,6,6,8,8]]
for t in range(len(sig)):
  ax = plt.subplot2grid((10,6),(locs[1][t], locs[0][t]), colspan=2)
  plt.sca(ax)
  vals_x, vals_y = [], []
  for sample in ft.columns:
    group = md.loc[sample, 'Time_to_progression_grouped']
  ...
  xl = plt.xlabel('Years to progression')
  #plt.yscale('symlog')
  ax = plt.subplot2grid((10,6),(locs[1][t], locs[0][t]-1))
  plt.sca(ax)
  vals_x, vals_y = [], []
  groups = {}
  for sample in ft.columns:
...
for t in range(len(sig)):
  ...
  ti = plt.title(sig[t], fontweight='bold')
  xl = plt.xticks([0,1],['P', 'NP'])
  if locs[0][t] == 1: yl = plt.ylabel(title_name)


plt.tight_layout()
plt.savefig(folder+'figures/FigureS4_div_'+div+'.png', dpi=600, bbox_inches='tight')
...
for t in range(len(sig)):
  ax = plt.subplot2grid((10,6),(t, 1), colspan=2)
  plt.sca(ax)
  vals_x, vals_y = [], []
  for sample in ft.columns:
    group = md.loc[sample, 'Time_to_progression_grouped']
...
#locs[1] = [1,1,3,3,5,5,7,7,9,9]
for t in range(len(sig)):
  ax = plt.subplot2grid((10,6),(t, 4), colspan=2)
  plt.sca(ax)
  vals_x, vals_y = [], []
  for sample in ft.columns:
    group = md.loc[sample, 'Time_to_progression_grouped']
...
plt.tight_layout()
plt.savefig(folder+'figures/FigureS4_div_'+div+'_flipped.png', dpi=600, bbox_inches='tight')

```

Rotated and split by group:
```{python}
sig_hits = {}
